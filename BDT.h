...
public:
    ...
    string name, gender, hairColor;
    string id;
    
    string input;
    cout<<"Is the answer correct? (yes/no) ";
    cin>>input;
    ...
    cout << "Enter ID of the character: ";
    cin >> id;
    ...
    cout << "How many tags does it have?: ";
    cin >> tagsCount;
    vector<string> tags(tagsCount);
    cin.ignore();
    ...
    cout << "How many anime appearances?: ";
    cin >> animeCount;
    vector<string> anime(animeCount);
    cin.ignore();

    for (int i = 0; i < animeCount; i++) {
        cout << "Enter Anime " << i + 1 << ": ";
        getline(cin, anime[i]);
    }

    cout << "How many manga appearances?: ";
    cin >> mangaCount;
    vector<string> manga(mangaCount);
    cin.ignore();

    for (int i = 0; i < mangaCount; i++) {
        cout << "Enter Manga " << i + 1 << ": ";
        getline(cin, manga[i]);
    }

    write << id << "," << name << "," << hairColor << "," << gender << ",";

    // Tags
    if (!tags.empty()) {
        for (size_t i = 0; i < tags.size(); ++i) {
            write << tags[i];
            if (i != tags.size() - 1) write << ".";
        }
    }
    write << ",";
    ...
    // Anime
    if (!anime.empty()) {
        for (size_t i = 0; i < anime.size(); ++i) {
            write << anime[i];
            if (i != anime.size() - 1) write << ".";
        }
    }
    write << ",";
